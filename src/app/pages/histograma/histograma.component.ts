...
export class HistogramaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.northwindService.getSelectsData().subscribe((selectsData: any) => {
  ...
  updateGraphic() {
    ...
    this.northwindService.getGraphicsData(dimension ? dimension : 'Cliente', body).subscribe((graphic: any) => {
        ...
        const labels = graphic.datosTabla.map(
          label => `${label.meses} ${label.años}`).filter((item, index, arr) => arr.indexOf(item) === index);
